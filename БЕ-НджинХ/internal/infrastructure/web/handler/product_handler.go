...
func (p *ProductHandler) AddProduct(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.service.CreateProduct(ctx, product, image); err != nil {
		respondJSON(w, http.StatusInternalServerError, "failed to create product")
		return
	}
...
func (p *ProductHandler) SubtractProduct(w http.ResponseWriter, r *http.Request) {
	...
	if req.Amount <= 0 {
		respondJSON(w, http.StatusBadRequest, "amount must be positive")
		return
	}
	...
	respondJSON(w, http.StatusOK, "amount decreased successfully")
}
...
func (p *ProductHandler) GetProduct(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, pkg.ErrIDNotFound) {
			respondJSON(w, http.StatusNotFound, "product not found")
			return
		}
		respondJSON(w, http.StatusInternalServerError, "fail to fetch product")
		return
	}
...
func (p *ProductHandler) GetAllProducts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondJSON(w, http.StatusInternalServerError, "fail to fetch products")
		return
	}
...
func (p *ProductHandler) DeleteProduct(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.repo.DeleteProduct(ctx, id); err != nil {
		...
		respondJSON(w, http.StatusInternalServerError, "failed to delete product")
		return
	}

	respondJSON(w, http.StatusOK, "product deleted successfully")
}
