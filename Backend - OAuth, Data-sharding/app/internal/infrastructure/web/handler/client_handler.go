...
func (c *ClientHandler) AddClient(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.RespondJSON(w, http.StatusBadRequest, err)
		return
	}
	...
	if err := c.repo.AddClient(ctx, client); err != nil {
		...
		case errors.Is(err, pkg.ErrClientAlreadyExists):
			utils.RespondJSON(w, http.StatusConflict, "client already exists")
		default:
			utils.RespondJSON(w, http.StatusInternalServerError, "failed to add client")
		}
		return
	}
...
func (c *ClientHandler) DeleteClient(w http.ResponseWriter, r *http.Request) {
	...
	if err := c.repo.DeleteClient(ctx, id); err != nil {
		if errors.Is(err, pkg.ErrIDNotFound) {
			utils.RespondJSON(w, http.StatusNotFound, "client not found")
			return
		}
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to delete client")
		return
	}

	utils.RespondJSON(w, http.StatusOK, "client deleted successfully")
}
...
func (c *ClientHandler) GetClient(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" || surname == "" {
		utils.RespondJSON(w, http.StatusBadRequest, "name and surname are required")
		return
	}
	...
	if err != nil {
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to fetch client")
		return
	}
...
func (c *ClientHandler) GetAllClients(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to fetch clients")
		return
	}
...
func (c *ClientHandler) AddressChange(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to update client address")
		return
	}
