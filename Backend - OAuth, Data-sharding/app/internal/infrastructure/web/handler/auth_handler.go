...
func (a *AuthHandler) GoogleLogin(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	authURL, err := a.client.GoogleLogin(ctx)
	if err != nil {
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to get auth url")
		return
	}
	utils.RespondJSON(w, http.StatusOK, map[string]string{"auth_url": authURL})
}
...
func (a *AuthHandler) GoogleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if code == "" || state == "" {
		utils.RespondJSON(w, http.StatusBadRequest, "code and state are required")
		return
	}

	ctx := r.Context()
	token, err := a.client.GoogleCallback(ctx, code, state)
	if err != nil {
		utils.RespondJSON(w, http.StatusInternalServerError, "failed to authenticate with google")
		return
	}
...
func (a *AuthHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/v1/auth/google/login", a.GoogleLogin)
