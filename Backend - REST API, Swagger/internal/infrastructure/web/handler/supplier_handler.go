...
func (s *SupplierHandler) AddSupplier(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.repo.AddSupplier(ctx, supplier); err != nil {
		...
		case errors.Is(err, pkg.ErrSupplierAlreadyExists):
			respondJSON(w, http.StatusConflict, "supplier already exists")
		default:
			respondJSON(w, http.StatusInternalServerError, "failed to add supplier")
		}
		return
	}

	respondJSON(w, http.StatusCreated, "supplier added successfully")
}
...
func (s *SupplierHandler) DeleteSupplier(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.repo.DeleteSupplier(ctx, id); err != nil {
		if errors.Is(err, pkg.ErrIDNotFound) {
			respondJSON(w, http.StatusNotFound, "supplier not found")
			return
		}
		respondJSON(w, http.StatusInternalServerError, "failed to delete supplier")
		return
	}

	respondJSON(w, http.StatusOK, "supplier deleted successfully")
}
...
func (s *SupplierHandler) GetSupplier(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respondJSON(w, http.StatusInternalServerError, "fail to fetch supplier")
		return
	}
...
func (s *SupplierHandler) GetAllSuppliers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondJSON(w, http.StatusInternalServerError, "fail to fetch suppliers")
		return
	}
...
func (s *SupplierHandler) AddressChange(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respondJSON(w, http.StatusInternalServerError, "failed to update supplier address")
		return
	}
