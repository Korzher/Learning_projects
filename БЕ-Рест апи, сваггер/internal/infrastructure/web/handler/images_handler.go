...
func (i *ImageHandler) AddImage(w http.ResponseWriter, r *http.Request) {
	...
	if req.ProductID == uuid.Nil {
		respondJSON(w, http.StatusBadRequest, "product_id is required")
		return
	}

	if len(req.Image) == 0 {
		respondJSON(w, http.StatusBadRequest, "image data is empty")
		return
	}
	...
	if err != nil {
		respondJSON(w, http.StatusInternalServerError, "failed to add image to product")
		return
	}

	respondJSON(w, http.StatusCreated, "image added and assigned to product successfully")
}
...
func (i *ImageHandler) ChangeImage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondJSON(w, http.StatusBadRequest, "invalid id format")
		return
	}
	...
	if len(req.Image) == 0 {
		respondJSON(w, http.StatusBadRequest, "image is empty")
		return
	}
	...
	if err != nil {
		if errors.Is(err, pkg.ErrImageNotFound) {
			respondJSON(w, http.StatusNotFound, "image not found")
			return
		}
		respondJSON(w, http.StatusInternalServerError, "failed to change image")
		return
	}

	respondJSON(w, http.StatusOK, "image changed successfully")
}
...
func (i *ImageHandler) DeleteImage(w http.ResponseWriter, r *http.Request) {
	...
	id, err := uuid.Parse(idStr)
	if err != nil {
		respondJSON(w, http.StatusBadRequest, "invalid image id")
		return
	}
	...
	if err != nil {
		...
		respondJSON(w, http.StatusInternalServerError, "failed to delete image")
		return
	}

	respondJSON(w, http.StatusOK, "image deleted successfully")
}
...
func (i *ImageHandler) GetImageByPID(w http.ResponseWriter, r *http.Request) {
	...
	id, err := uuid.Parse(idStr)
	if err != nil {
		respondJSON(w, http.StatusBadRequest, "invalid product id")
		return
	}
	...
	if err != nil {
		...
		respondJSON(w, http.StatusInternalServerError, "failed to retrieve image")
		return
	}

	respondImage(w, http.StatusOK, image)
}
